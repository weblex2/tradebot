...
<div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <div class="w-64 flex flex-col flex-1 min-h-0">
        ...
        <nav class="flex-1 p-4 space-y-1">
            ...
            <a href="{{ route('tradebot.chat') }}"
               class="sidebar-link {{ request()->routeIs('tradebot.chat') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>
                </svg>
                AI Chat
            </a>
            <a href="{{ route('tradebot.fixes') }}"
               class="sidebar-link {{ request()->routeIs('tradebot.fixes') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                Auto Fixes
            </a>
            <a href="{{ route('tradebot.discussions') }}"
               class="sidebar-link {{ request()->routeIs('tradebot.discussions') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('tradebot.prompts') }}"
               class="sidebar-link {{ request()->routeIs('tradebot.prompts') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/>
                </svg>
                Prompts
            </a>
            <a href="{{ route('tradebot.docs') }}"
               class="sidebar-link {{ request()->routeIs('tradebot.docs') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                </svg>
                Docs
            </a>
            <a href="{{ route('tradebot.settings') }}"
               class="sidebar-link {{ request()->routeIs('tradebot.settings') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
